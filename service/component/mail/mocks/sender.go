...
func (m *Sender) SendTemplate(_ *trixorm.Engine, message *mail.Message) error {
	return m.Called(message.To).Error(0)
}

func (m *Sender) SendTemplateWithAttachments(_ *trixorm.Engine, message *mail.MessageAttachment) error {
	return m.Called(message.To).Error(0)
}

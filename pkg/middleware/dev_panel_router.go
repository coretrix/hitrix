...
func DevPanelRouter(ginEngine *gin.Engine) {
	...
	var devPanel *controller.DevPanelController
	{
		...
		}
		{
			...
			devGroup.GET("redis-delayed-queues/list/", devPanel.GetRedisDelayedQueues)
	...
	var errorLog *controller.ErrorLogController
	{
		...
		errorLogGroup.GET("services/", errorLog.GetServices)
		errorLogGroup.GET("services/remove/:id/", errorLog.DeleteService)
		errorLogGroup.GET("services/remove-all/", errorLog.DeleteAllServices)
		errorLogGroup.POST("services/create-jira-ticket/:id/", errorLog.CreateServiceJiraTicket)
